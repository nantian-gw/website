...
## HTTPS with Fallback Certificate

If you need HTTPS without managing certificates, Nantian Gateway automatically provisions a self-signed certificate. This is ideal for development or internal environments.
...
spec:
  ...
  - name: https
    ...
    tls:
      mode: Terminate
      certificateRefs:
      - name: dont-exist   # Gateway API requires a name; gateway substitutes fallback cert
    allowedRoutes:
      namespaces:
        from: All
```

The control plane creates a self-signed CA (stored in `Secret/nantian-gw-fallback-ca`), issues a leaf certificate for the listener, and pushes it to data planes — zero configuration beyond the Gateway manifest.

:::caution
Fallback certificates are **not suitable for production**. They provide TLS encryption but no trust. Use [cert-manager](#https-with-cert-manager) or manual certificate provisioning for production deployments.
:::
...
Verify the backend's certificate against a custom CA. The CA can be stored in a **ConfigMap** or **Secret**.

```bash
# Option A: ConfigMap (recommended for simplicity)
kubectl create configmap backend-ca -n default --from-file=ca.crt=/path/to/ca.pem

# Option B: Secret
kubectl create secret generic backend-ca-secret -n default --from-file=ca.crt=/path/to/ca.pem
...
spec:
  ...
  validation:
    ...
    - name: backend-ca
      group: ""
      kind: ConfigMap      # or "Secret" for Secret-based CA
    subjectAltNames:
