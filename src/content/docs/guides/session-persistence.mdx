...
## Troubleshooting: Sessions Keep Dropping
...
| nginx-ingress annotation | nantian-gw equivalent |
|---|---|
| `nginx.ingress.kubernetes.io/affinity: cookie` | Create a `BackendLBPolicy` with `sessionPersistence.type: Cookie` |
...
| `nginx.ingress.kubernetes.io/session-cookie-path` | Not applicable (cookie path is `/` by default) |
