---
title: Admin API
description: "Internal design of the control plane Admin HTTP API — route contracts, auth model, readiness modes, and how the dashboard and tooling consume it."
---
...
Every Admin API request is recorded in structured audit logs. The audit middleware sits after authentication in the request chain, so caller identity is available for every logged event.

### Log format

Audit events are written to stdout as JSON (or text, depending on `logging.format`). Each event includes:

| Field | Description |
|-------|-------------|
| `log_type` | Always `audit` — use this to filter or route audit events |
| `who` | Authenticated caller identity |
| `method` | HTTP method (`GET`, `POST`, `PUT`, `DELETE`) |
| `path` | Request URL path |
| `route` | Normalized route name (e.g., `listeners`, `resource_detail`) |
...
The `who` field is resolved from the authentication layer:

| Auth mode | `who` value |
|-----------|------------|
| `kubernetes` | Kubernetes username from TokenReview (e.g., `system:admin`) |
| `static` | `static-token` |
| `none` | `anonymous` |

### Log levels

Audit events use log levels based on the response status:

| Status | Level | Example scenario |
|--------|-------|-----------------|
| 2xx | `INFO` | Successful read or mutation |
| 4xx | `WARN` | Invalid request, unauthorized, not found |
