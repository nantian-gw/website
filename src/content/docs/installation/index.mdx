...
description: "How to install Nantian Gateway with Helm or Kustomize, what CRDs you need first, and how to verify the release."
---
...
| --- | --- | --- |
| [Helm](/installation/helm/) | Most installations, upgrades, and value overrides. | Uses the chart repository at `https://chart.nantian.dev`. |
| [Kustomize](/installation/kustomize/) | GitOps or repository-local manifest customization. | Uses overlays from the component repositories. |
...
| Dashboard Deployment | `nantian-gw-dashboard` | 2 replicas | Web UI for users and operators. |
...
- The chart defaults to `latest` image tags with `pullPolicy: IfNotPresent` for control plane, data plane, and dashboard; production environments should pin a specific release tag or digest explicitly in their values files.
...
1. Start with [Helm Installation](/installation/helm/) for repository setup, render/install commands, release verification, and `helm test`.
2. Use [Helm Values Guide](/configuration/helm-values/) to organize environment-specific values files and choose the right value groups.
3. Apply the hardening checklist in [Production Deployment](/installation/production/) before exposing production traffic.
4. Follow [Upgrade Guide](/installation/upgrade/) when chart versions, values schemas, or CRD expectations change.
...
Then create a demo `Gateway` and `HTTPRoute` as shown in the [Quick Start](/getting-started/quick-start/).

## Choosing The Next Page

- Use [Helm Installation](/installation/helm/) for repository setup, default values, render/install commands, `helm test`, and uninstall commands.
- Use [Helm Values Guide](/configuration/helm-values/) for layered values files, major chart value groups, and production guardrails.
- Use [Experimental Features](/configuration/experimental-features/) before creating `AIService`, `TokenPolicy`, `WasmPlugin`, or experimental `BackendLBPolicy` resources.
- Use [Operations](/operations/) for first checks after install, service names, logs, metrics, and admin access.

## Air-Gapped and Offline Installations
