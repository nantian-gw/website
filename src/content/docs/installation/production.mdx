...
description: "Production checklist for Nantian Gateway: resource sizing, TLS setup, high availability, monitoring, backup strategy, and security hardening recommendations."
---
...
controlplane:
  grpcTLS:
    enabled: true
    existing_secret: "nantian-grpc-tls"
    requireClientCert: true

dataplane:
  xdsTLS:
    enabled: true
    domain_name: "nantian-gw-controlplane-grpc.nantian-gw.svc.cluster.local"
```
...
**Downstream TLS** for traffic entering the gateway. This is configured through Gateway API listeners, not the Helm chart. See the [TLS configuration guide](/configuration/tls) for details.
...
The data plane sets `net.ipv4.ip_unprivileged_port_start=0` via a pod-level sysctl, which allows non-root users to bind to any port (including privileged ports below 1024) without requiring the `NET_BIND_SERVICE` capability. This is the recommended approach on Kubernetes clusters that allow unsafe sysctls. All capabilities are dropped for both components. When `hostNetwork` is enabled, the sysctl is skipped since low ports are natively accessible.
...
`non_blocking: true` keeps the data plane from stalling on slow log writes. `drop_when_full: true` discards log lines rather than blocking under extreme load.
...
Use immutable release tags or image digests in production. Avoid `latest`, because it makes rollbacks and audits ambiguous.
