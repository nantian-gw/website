...
If your cluster cannot reach GitHub, download the manifest first and transfer it. See [Air-Gapped Installation](/installation/#air-gapped-and-offline-installations).
:::
...
`featureMode` controls Nantian Gateway runtime feature gates. Set to `experimental` to enable AI Gateway, Wasm, TokenPolicy, and RoutePolicy features.
`gatewayAPI.installCRDs` controls whether this chart renders official Gateway API CRDs. When `true`, the chart installs the standard Gateway API CRDs automatically.
...
controlplane:
  ...
  image:
    ...
    # tag: "v2026.08.0-alpha.1"  # defaults to appVersion
...
The image tag defaults to the chart's `appVersion`. Override with a specific release tag or an image digest for production.
Release tags are published to `ghcr.io/nantian-gw/` (public, no auth needed).
...
The chart defaults to `latest` image tags for control plane, data plane, and dashboard. Production environments should pin a specific release tag or digest explicitly in their values files.
