...
The Helm chart ships with a `production-values.yaml` that covers most of these items. Use it as a starting point, then layer your environment-specific overrides on top.
...
The production values file intentionally omits a CPU limit on the data plane. This lets the data plane burst to use available CPU on the node during traffic spikes. The memory limit is the hard cap that prevents a runaway proxy from exhausting node memory.
:::
...
Helm merges the files in order, with later files overriding earlier ones. This lets you keep `production-values.yaml` as the shared baseline and `my-environment.yaml` for cluster-specific settings.
