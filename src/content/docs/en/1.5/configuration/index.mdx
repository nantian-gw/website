...
The control plane reads its configuration from a file you specify with the `--config` flag at startup. The default path depends on your deployment method: Helm charts mount a ConfigMap at `/etc/nantian/config.yaml`, while local development picks up `configs/controlplane/config.yaml` from the repository.
...
2. **Helm ConfigMap mount** — `/etc/nantian/config.yaml`, provided by the Helm chart.
