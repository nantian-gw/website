...
Each route type can be attached to one or more Gateways, and multiple routes can coexist on the same listener. For details on every resource field, see the [Gateway API Resources](/en/1.5/api-reference/gateway-api) reference.
...
Enable the relevant experimental feature flags before creating these resources. See the [Custom And Experimental Resources](/en/1.5/api-reference/crds) reference for current fields and YAML examples.
...
This push-based model means configuration changes propagate to all data planes within seconds, with no polling overhead. For the protocol specification, message formats, and field definitions, see [xDS Protocol](/en/1.5/api-reference/xds-proto).

## Where to Start

If you only need to configure gateway routing, start with [Gateway API Resources](/en/1.5/api-reference/gateway-api). This API follows the Kubernetes Gateway API specification, so your `HTTPRoute`, `Gateway`, and other configs work exactly the same as with any other compatible implementation.

If you need to manage AI model routing, token quotas, Wasm plugins, or custom load balancing policies, continue to the [Custom CRDs](/en/1.5/api-reference/crds) section. Nantian Gateway's four CRDs (`AIService`, `TokenPolicy`, `WasmPlugin`, `BackendLBPolicy`) extend the standard Gateway API, letting you manage these advanced features with the same Kubernetes native workflow.

For troubleshooting, the [Admin API](/en/1.5/api-reference/admin-api) is your go-to entry point. It exposes the internal state of both the control plane and data plane, including config snapshot versions, xDS client connection status, and backend health check results.

If you're developing Nantian Gateway itself or need to understand the communication protocol between the control plane and data plane, the [xDS Protocol](/en/1.5/api-reference/xds-proto) section describes the message formats and field definitions carried over the gRPC bidirectional stream in detail.
...
* See [Gateway API Resources](/en/1.5/api-reference/gateway-api) for standard routing configuration
* See [Custom CRDs](/en/1.5/api-reference/crds) for Nantian Gateway's extension capabilities
* See [Admin API](/en/1.5/api-reference/admin-api) for runtime diagnostics endpoints
* See [xDS Protocol](/en/1.5/api-reference/xds-proto) for the control plane/data plane communication protocol
