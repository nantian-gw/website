...
The admin API provides full read and write access to gateway resources. Never expose `adminAddr` to external networks without TLS and authentication. See the [TLS configuration](/en/configuration/tls) page for securing it.
:::
...
Refer to the [TLS / mTLS](/en/configuration/tls) page for detailed TLS configuration.
...
- [Data Plane Configuration](/en/configuration/dataplane) — data plane config reference
- [TLS / mTLS](/en/configuration/tls) — detailed TLS configuration guide
- [Observability](/en/configuration/observability) — logging, metrics, and tracing configuration