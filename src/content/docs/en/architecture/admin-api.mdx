...
:::caution
The Admin API provides full read access to your gateway configuration, including secrets, backend addresses, and route details. Never expose `adminAddr` to external networks without TLS and authentication. See [TLS / mTLS](/en/configuration/tls) for configuration details.
:::
...
- [API Reference: Admin API](/en/api-reference/admin-api) -- OpenAPI specification and client libraries
- [Operations: Metrics Reference](/en/operations/metrics) -- complete Prometheus metrics catalog
- [Configuration: Control Plane](/en/configuration/controlplane) -- admin server configuration