...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `adminAddr` | string | `"127.0.0.1:19080"` | Admin listener address; serves `/metrics` alongside health endpoints |
...
OpenTelemetry tracing is optional and is configured under `log.openTelemetry` in the data plane configuration:

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `log.openTelemetry.enabled` | bool | `false` | Enable OpenTelemetry tracing |
| `log.openTelemetry.endpoint` | string | `""` | OTLP collector endpoint |
| `log.openTelemetry.protocol` | string | `"grpc"` | OTLP transport (`grpc` or `http`) |
| `log.openTelemetry.timeoutMs` | int | `3000` | Export timeout in milliseconds |
| `log.openTelemetry.insecure` | bool | `false` | Disable TLS when connecting to the collector |
| `log.openTelemetry.sampleRatio` | float | `1.0` | Sampling ratio (0.0 to 1.0) |
| `log.openTelemetry.serviceName` | string | `"nantian-dataplane"` | Service name in trace data |
| `log.openTelemetry.serviceNamespace` | string | `""` | Service namespace in trace data |
