...
description: "Complete reference for the Go control plane config file: server addresses, controller settings, reconciler runner, node status, TLS, gRPC, and feature flags."
---
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `nodeStatus.leasePrefix` | string | `nantian-gw-node` | Prefix for node lease objects in the Kubernetes API |
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `adminAuth.authMode` | string | `"static"` | Authentication mode: `"static"` or `"kubernetes"` |
| `adminAuth.bearerToken` | string | `""` | Static bearer token for admin API authentication. Only used when `authMode` is `"static"`. |
| `adminAuth.bearerTokenFile` | string | `""` | Path to a file containing the bearer token. Only used when `authMode` is `"static"`. |
| `adminAuth.rateLimitRPS` | int64 | `0` | Requests per second rate limit for the admin API (0 = unlimited) |

**Static mode** (`authMode: "static"`, default): When both `bearerToken` and `bearerTokenFile` are empty, the admin API allows read access but blocks writes. When configured, all requests must include an `Authorization: Bearer <token>` header.

**Kubernetes mode** (`authMode: "kubernetes"`): Validates bearer tokens against the Kubernetes [TokenReview](https://kubernetes.io/docs/reference/kubernetes-api/authentication-resources/token-review-v1/) API. This lets operators authenticate using tokens generated by `kubectl create token <service-account>` instead of a pre-configured static token. The control plane ServiceAccount needs permission to create `tokenreviews` (the Helm chart grants this automatically).
