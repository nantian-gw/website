...
The proxy runtime does not expose TLS session-cache tuning knobs. TLS protocol version bounds (`runtime.tlsMinVersion` / `runtime.tlsMaxVersion`) and certificate/mTLS settings are covered in [TLS Configuration](/configuration/tls/).
