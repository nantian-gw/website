...
The resolved policy appears in the data plane snapshot under `BackendCluster.loadBalancing`, `BackendCluster.sessionPersistence`, and `BackendCluster.circuitBreaker`. You can inspect these fields through the data plane admin API at the `/snapshots` endpoint.
