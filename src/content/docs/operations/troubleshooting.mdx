...
description: "Diagnose common Nantian Gateway issues: control plane crashes, snapshot build failures, data plane disconnections, route misconfiguration, and resource conflicts."
---
...
- **Admin auth token resolution failed**: If `admin_auth.bearer_token_file` points to a non-existent file or `admin_auth.bearer_token` is configured but the env var is empty, startup fails.
...
**Symptoms**: Data plane logs repeatedly show `xds stream superseded by newer connection`, while control plane logs alternate between `dataplane connected` and draining the older stream for the same node.
...
- **Duplicate runtime `node_id` across replicas**: The control plane keeps only one active xDS stream per `node_id`, so a newer connection replaces the older one.
- **Old manifests still use a fixed `dataplane.config.node_id`**: Static values such as `dp-kubernetes` are safe only as fallbacks when the runtime also receives a unique env override.
- **Obsolete env wiring**: Older manifests may still inject `PGW_NODE_ID`. Current chart versions inject `NANTIAN_GW_NODE_ID` from Pod `metadata.name`.

**Resolution**: Ensure every data plane replica has a unique runtime identity. With the current Helm chart, keep the `NANTIAN_GW_NODE_ID <- metadata.name` injection and avoid overriding it with a shared fixed value. If you maintain custom manifests, remove `PGW_NODE_ID`, keep `dataplane.config.node_id` only as a fallback, and restart the data plane after applying the corrected Deployment.

### Session persistence warning about auto-generated keys

**Symptoms**: Data plane logs show warnings such as `session persistence is using an ephemeral, auto-generated secret` or `configure shared_secret or sharedSecretFile`.

**Resolution**: Configure `dataplane.session_persistence.existing_secret` or `dataplane.session_persistence.shared_secret` when running more than one replica. Current Helm chart releases mount that Secret and wire the runtime `session_persistence.secret_key_file` automatically, so sticky-session state survives restarts and can be shared across replicas.
...
- [Alerting Rules](/operations/alerting) -- configure alerts to catch issues before they escalate
- [Configuration](/configuration/) -- review configuration options that affect troubleshooting
