...
- **Obsolete env wiring**: Older manifests may still inject `PGW_NODE_ID`. Current chart versions inject `AEG_NODE_ID` from Pod `metadata.name`.

**Resolution**: Ensure every data plane replica has a unique runtime identity. With the current Helm chart, keep the `AEG_NODE_ID <- metadata.name` injection and avoid overriding it with a shared fixed value. If you maintain custom manifests, remove `PGW_NODE_ID`, keep `dataplane.config.nodeId` only as a fallback, and restart the data plane after applying the corrected Deployment.
