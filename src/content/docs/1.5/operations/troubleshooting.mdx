...
description: "Diagnose and resolve common Nantian Gateway issues including control plane failures, data plane connectivity, route matching, and TLS certificate problems."
slug: 1.5/operations/troubleshooting
---
...
* **Duplicate runtime `nodeId` across replicas**: The control plane keeps only one active xDS stream per `nodeId`, so a newer connection replaces the older one.
* **Old manifests still use a fixed `dataplane.config.nodeId`**: Static values such as `dp-kubernetes` are safe only as fallbacks when the runtime also receives a unique env override.
...
**Resolution**: Ensure every data plane replica has a unique runtime identity. With the current Helm chart, keep the `NANTIAN_GW_NODE_ID <- metadata.name` injection and avoid overriding it with a shared fixed value. If you maintain custom manifests, remove `PGW_NODE_ID`, keep `dataplane.config.nodeId` only as a fallback, and restart the data plane after applying the corrected Deployment.

### Session persistence warning about auto-generated keys

**Symptoms**: Data plane logs show warnings such as `session persistence is using an ephemeral, auto-generated secret` or `configure sharedSecret or sharedSecretFile`.

**Resolution**: Configure `dataplane.sessionPersistence.existingSecret` or `dataplane.sessionPersistence.sharedSecret` when running more than one replica. Current Helm chart releases mount that Secret and wire the runtime `sessionPersistence.secretKeyFile` automatically, so sticky-session state survives restarts and can be shared across replicas.
...
* [Backup & Recovery](/1.5/operations/backup) -- procedures for disaster recovery
* [Alerting Rules](/1.5/operations/alerting) -- configure alerts to catch issues before they escalate
* [Configuration](/1.5/configuration/) -- review configuration options that affect troubleshooting
