...
description: "Overview of Nantian Gateway installation methods, component architecture, service ports, and default Helm chart behavior."
slug: 1.5/installation
---
...
| --- | --- | --- |
| [Helm](/1.5/installation/helm/) | Most installations, upgrades, and value overrides. | Uses the chart repository at `https://chart.nantian.dev`. |
| [Kustomize](/1.5/installation/kustomize/) | GitOps or repository-local manifest customization. | Uses overlays from the component repositories. |
...
Then create a demo `Gateway` and `HTTPRoute` as shown in the [Quick Start](/1.5/getting-started/quick-start/).

## Choosing The Next Page

* Use [Helm Installation](/1.5/installation/helm/) for repository setup, default values, common overrides, and uninstall commands.
* Use [Experimental Features](/1.5/configuration/experimental-features/) before creating `AIService`, `TokenPolicy`, `WasmPlugin`, or experimental `BackendLBPolicy` resources.
* Use [Operations](/1.5/operations/) for first checks after install, service names, logs, metrics, and admin access.
