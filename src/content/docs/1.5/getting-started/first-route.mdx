...
description: "Learn how to create a Gateway, HTTPRoute, and backend service to route traffic through Nantian Gateway with path-based routing."
slug: 1.5/getting-started/first-route
---
...
* **Installation options**: Learn about [Helm customization](/1.5/installation/helm), [Kustomize overlays](/1.5/installation/kustomize), and [production deployment](/1.5/installation/production).
* **TLS configuration**: Set up [TLS listeners and mTLS](/1.5/configuration/tls) for secure traffic.
* **Observability**: Enable [metrics scraping](/1.5/operations/metrics) and set up [Grafana dashboards](/1.5/operations/grafana).
* **Architecture**: Read the [architecture overview](/1.5/architecture/) to understand the control plane and data plane split.
* **API reference**: Browse the [Gateway API resources](/1.5/api-reference/gateway-api) and [custom CRDs](/1.5/api-reference/crds).
