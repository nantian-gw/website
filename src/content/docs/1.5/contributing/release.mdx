...
Nantian Gateway follows the [Semantic Versioning 2.0.0](https://semver.org/) specification. The version number format is `MAJOR.MINOR.PATCH`:
...
The project is split across component repositories. A stable release uses one product version such as `v1.3.0` for the controlplane, dataplane, and dashboard image tags. The Helm chart uses chart version `1.3.0` and records the product version in `appVersion: "v1.3.0"`. Development image tags such as `latest`, `sha-*`, and `ci-*` are for validation and should not replace stable release tags.
