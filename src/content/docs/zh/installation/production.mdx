...
:::tip[数据面不设 CPU 上限]
生产配置特意不给数据面设 CPU 限制。这样流量尖峰时数据面能 burst 吃满节点上可用的 CPU。内存限制是硬上限，防止一个跑飞的代理吞掉节点内存。
:::
...
数据面通过 Pod 级 sysctl 设置 `net.ipv4.ip_unprivileged_port_start=0`，允许非 root 用户绑定任意端口（包括 1024 以下的特权端口），无需 `NET_BIND_SERVICE` capability。这是支持 unsafe sysctl 的 Kubernetes 集群上的推荐做法。所有 capabilities 全部丢弃。当启用 `hostNetwork` 时跳过该 sysctl，因为低端口可直接访问。
...
`non_blocking: true` 防止数据面因慢速日志写入而卡顿。`drop_when_full: true` 在极端负载下丢弃日志行而不是阻塞线程。
...
把这些占位符替换成你已经同步到私有仓库的不可变 tag 或 digest。不要使用 `latest`，否则回滚和审计都会变得不明确。
