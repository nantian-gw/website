...
Admin API 提供对网关资源的完整读写访问。切勿在未配置 TLS 和认证的情况下将 `adminAddr` 暴露到外部网络。请参阅 [TLS 配置](/zh/configuration/tls) 页面了解安全配置方法。
:::
...
| 参数 | 类型 | 默认值 | 说明 |
|-----------|------|---------|-------------|
| `nodeStatus.leasePrefix` | string | `nantian-gw-node` | Kubernetes API 中节点租约对象的前缀 |
...
| 参数 | 类型 | 默认值 | 说明 |
|-----------|------|---------|-------------|
| `adminAuth.authMode` | string | `"static"` | 认证模式：`"static"` 或 `"kubernetes"` |
| `adminAuth.bearerToken` | string | `""` | Admin API 认证的静态 bearer token。仅在 `authMode` 为 `"static"` 时使用。 |
| `adminAuth.bearerTokenFile` | string | `""` | 包含 bearer token 的文件路径。仅在 `authMode` 为 `"static"` 时使用。 |
| `adminAuth.rateLimitRPS` | int64 | `0` | Admin API 每秒请求数限制（0 = 无限制） |

**静态模式** (`authMode: "static"`，默认)：当 `bearerToken` 和 `bearerTokenFile` 均为空时，Admin API 允许读取但禁止写入。配置后，所有请求必须包含 `Authorization: Bearer <token>` 请求头。

**Kubernetes 模式** (`authMode: "kubernetes"`)：通过 Kubernetes [TokenReview](https://kubernetes.io/docs/reference/kubernetes-api/authentication-resources/token-review-v1/) API 验证 bearer token。运维人员可以使用 `kubectl create token <service-account>` 生成的 token 进行认证，无需预设静态 token。控制面 ServiceAccount 需要创建 `tokenreviews` 的权限（Helm Chart 自动授予）。
...
详细 TLS 配置请参阅 [TLS / mTLS](/zh/configuration/tls) 页面。
...
- [数据面配置](/zh/configuration/dataplane) — 数据面配置参考
- [TLS / mTLS](/zh/configuration/tls) — 详细的 TLS 配置指南
- [可观测性](/zh/configuration/observability) — 日志、指标和链路追踪配置
