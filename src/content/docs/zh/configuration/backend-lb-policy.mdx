...
查询数据平面管理 API（端口 `19080`）可查看哪些策略处于活跃状态。`/snapshots` 端点返回完整的 IR 快照。找到与你的服务匹配的 `BackendCluster` 条目，检查其中的 `session_persistence`、`loadBalancing` 和 `circuitBreaker` 字段。仅当该后端已解析出 `BackendLBPolicy` 时，这些字段才会出现。
