...
4. **确认和应用**：数据面收到快照后解析并应用到本地基于 Pingora 的运行时，然后向控制面发回 ACK。此时数据面已用最新配置开始路由流量。

5. **配置更新**：当 Kubernetes 资源变更时（Gateway、HTTPRoute、Service、EndpointSlice 任一发生变化），控制面重建快照（内部可能是全量或部分重建），然后推送带新版本号的完整快照。数据面应用后再次发 ACK。线上传输的始终是全量快照，绝非增量。

6. **重连**：如果 gRPC 流因网络问题或控制面重启而断开，数据面以指数退避重新连接。重连时数据面上报自己最后应用的版本；若与当前版本不同，控制面推送当前全量快照，否则跳过推送。
...
下面是数据面（DP）与控制面（CP）从启动到配置更新的完整序列：
...
Nantian Gateway 以带版本的全量（SotW）快照交付配置：

**初始同步。** 数据面首次连接时，控制面构建并推送一份完整的 SotW 快照，覆盖所有资源类型：listener、cluster、route、endpoint。快照带有一个 nonce（唯一标识符），数据面必须在 ACK 中原样回传。

**快照推送。** 初始同步之后，控制面为每个已连接的数据面跟踪版本号。当 Kubernetes 资源变化时，快照构建器重建快照（内部可能是全量或部分重建），然后推送带新版本号的完整快照。版本跟踪让控制面可以跳过向已持有当前版本的数据面推送；线上传输的始终是全量快照，而非增量。
...
| `xds_tls.enabled` | bool | `false` | 是否给 xDS 传输加 TLS |
| `xds_tls.ca_path` | string | 空 | 验证控制面证书的 CA 证书路径 |
| `xds_tls.cert_path` | string | 空 | 客户端证书路径（mTLS） |
| `xds_tls.key_path` | string | 空 | 客户端私钥路径（mTLS） |

数据面启动后立即尝试连接控制面。如果连接失败，数据面以指数退避重试，直到连接成功。连接建立后，数据面发送节点标识和当前配置版本，控制面根据版本差异决定是否推送当前全量快照（版本一致时可跳过）。
...
TLS 握手失败通常是证书不匹配。检查 `xds_tls.ca_path` 是否指向了签发控制面证书的 CA。对于 mTLS 场景，确认 `xds_tls.cert_path` 和 `xds_tls.key_path` 都已设置，且控制面信任客户端的 CA。可以用 `openssl s_client -connect <cp-host>:<grpc-port>` 手动测试 TLS 握手。
