...
gRPC xDS 服务器（`gateway/internal/xds/server.go`）实现 `proto/gateway/control/v1/` 中定义的 `ConfigurationDiscoveryService` gRPC 服务。数据面通过双向流式 RPC 连接并交换 `DiscoveryRequest` 和 `DiscoveryResponse` 消息。
