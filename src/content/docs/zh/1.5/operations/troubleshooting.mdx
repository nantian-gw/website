---
title: 故障排查
slug: zh/1.5/operations/troubleshooting
---
...
```bash
# 通过控制面代理查询指定数据面节点
curl http://<controlplane>:18081/v1/dataplanes/<nodeId>/summary
```
...
4. 检查监听器是否就绪：
   ...
   ```bash
   curl http://<controlplane>:18081/v1/listeners
   ```
...
5. 检查后端端点是否健康：
   ...
   ```bash
   curl http://<controlplane>:18081/v1/backends/<namespace>/<name>
   ```
...
* 仍在使用已经废弃的 `PGW_NODE_ID`，而不是当前 chart 使用的 `NANTIAN_GW_NODE_ID`

**解决方法**：确保每个数据面副本都有唯一的运行时身份。当前 Helm chart 会把 Pod `metadata.name` 注入到 `NANTIAN_GW_NODE_ID`，通常不需要额外修改；不要再用共享固定值覆盖它。若你维护的是自定义清单，请移除 `PGW_NODE_ID`，把 `dataplane.config.nodeId` 仅作为兜底值保留，然后重启数据面 Deployment。
