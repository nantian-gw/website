---
title: 适用场景
description: "了解 Nantian Gateway 在 AI 网关、Kubernetes Ingress、多协议代理、无 Sidecar 服务网格、Wasm 扩展和高性能边缘网关中的典型部署模式。"
slug: zh/1.5/overview/use-cases
---

Nantian Gateway 的架构适用于多种不同的部署模式。本页介绍网关发挥独特价值的实际场景。
...
AI 提供商使用不同的 API 格式。OpenAI、Anthropic 和 Ollama 各自拥有独立的请求和响应模式。Nantian Gateway 规范化这些差异，让您的应用使用一种格式通信，而网关则将其转换为各后端期望的格式。
...
Nantian Gateway 的 `TokenPolicy` CRD 基于实际 token 使用量来执行限制：
...
将敏感数据发送到外部 AI 提供商会带来合规风险。Nantian Gateway 可以在提示词离开您的网络之前脱敏其中的个人身份信息。脱敏在代理层完成，因此无需额外的独立服务。

### A/B 测试模型部署

当上线新的模型版本或比较不同提供商时，您需要分流流量并对比结果。Nantian Gateway 支持在 AI 后端之间按百分比分流流量，使用与常规 HTTP 流量相同的路由原语：
...
:::caution
无 sidecar 的网格模型不提供每个 Pod 的 mTLS 或基于 SPIFFE 的身份认证。如果您需要这些功能，可考虑使用 Istio 或 Linkerd 作为网格层，Nantian Gateway作为 ingress。
:::

## Wasm 驱动的自定义扩展

当内置过滤器不够用时，Nantian Gateway 的 Wasm 插件系统让您能够在代理层注入自定义逻辑。
...
Nantian Gateway附带一个 [Wasm SDK](https://github.com/nantian-gw/gateway)（Rust crate: `ntgw-wasm-sdk`），为插件开发提供类型化绑定。您用 Rust 编写插件，编译到 `wasm32-wasi`，然后通过 CRD 部署。

## 高性能边缘与可观测性

对于延迟敏感的工作负载，Nantian Gateway 的 Rust 数据面提供了可预测的性能和低尾部延迟。结合可观测性技术栈，您能全面洞察网关行为。
...
完整过程请参阅[生产安装](/zh/1.5/installation/production/)指南。

## 组合用例

Nantian Gateway 的优势在于这些用例可以组合使用。单个部署可以：
...
- [快速开始：5 分钟部署](/zh/1.5/getting-started/quick-start/)
- [AI 网关功能指南](/zh/1.5/features/ai-gateway/)
- [Wasm 插件功能指南](/zh/1.5/features/wasm-plugins/)
- [生产部署指南](/zh/1.5/installation/production/)
