---
title: 项目对比
description: "从架构、功能覆盖、数据面、AI 网关和运维复杂度对比 Nantian Gateway 与 Envoy Gateway、Istio、Contour、NGINX Gateway Fabric。"
slug: zh/1.5/overview/comparison
---
...
| 维度 | Nantian Gateway | Envoy Gateway | Istio | Contour | NGINX Gateway |
|---------|----------------|---------------|-------|---------|---------------|
| **Gateway API 目标版本** | v1.5.1 | v1.2.x | v1.2.x（通过 Gateway API） | v1.1.x | v1.2.x |
| **声明支持特性数** | 54 | ~40 | ~30（Gateway API） | ~30 | ~35 |
| **控制面** | Go | Go | Go | Go | Go |
| **数据面** | Rust | C++（Envoy） | C++（Envoy） | C++（Envoy） | C（NGINX） |
| **配置协议** | gRPC/xDS | gRPC/xDS | gRPC/xDS | gRPC/xDS | 自定义 |
| **AI 网关** 🧪 | 内置 | 否 | 否 | 否 | 否 |
| **Wasm 运行时** 🧪 | wasmtime | wasmtime（proxy-wasm） | wasmtime（proxy-wasm） | wasmtime（proxy-wasm） | 否 |
| **服务网格** | 是（Gateway API Mesh） | 否 | 是（核心能力） | 否 | 否 |
| **TCP/UDP/TLS** | 是 | 是 | 是 | 是 | 是 |
| **gRPC 路由** | 是（命名规则） | 是 | 是 | 有限 | 是 |
| **仪表盘** | Next.js 管理台 | 无内置 | Kiali | 无内置 | 无内置 |
...
## Nantian Gateway 与 Envoy Gateway
...
## Nantian Gateway 与 NGINX Gateway Fabric
...
## 何时选择 Nantian Gateway

Nantian Gateway 非常适合以下场景：

- 你需要**全面的 Gateway API 覆盖**（v1.5.1, 54 项功能），并希望紧跟上游规范
