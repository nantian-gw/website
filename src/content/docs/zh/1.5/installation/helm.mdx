...
实验性 Gateway policy、AI、token policy 和 Wasm 行为默认关闭。使用实验 API 前，需要同时配置两个平面：

* 控制面：`controlplane.config.features.enableExperimentalGateway`
* 控制面：`controlplane.config.features.enableAiGateway`
* 数据面：`dataplane.config.experimental.enableExperimentalGateway`
* 数据面：`dataplane.config.experimental.enableAiGateway`
