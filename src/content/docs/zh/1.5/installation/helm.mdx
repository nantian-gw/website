...
chart 默认对控制面使用 `v2026.06.0` 发布 tag（`pullPolicy: IfNotPresent`）。生产环境仍建议在 values 文件里显式固定具体发布 tag 或 digest。
...
chart 默认对 controlplane、dataplane 和 dashboard 都使用 `v2026.06.0` 发布 tag（`pullPolicy: IfNotPresent`）。生产环境仍建议在 values 文件里显式固定具体发布 tag 或 digest。
