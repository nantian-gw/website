---
title: 可观测性
slug: zh/1.5/configuration/observability
---
...
数据面在其 Admin 监听器上暴露指标，与健康检查端点共用同一端口：

| 参数 | 类型 | 默认值 | 说明 |
|-----------|------|---------|-------------|
| `adminAddr` | string | `"127.0.0.1:19080"` | Admin 监听地址；在 `/metrics` 路径上提供指标 |

在 Kubernetes 中，Helm chart 通过数据面 metrics Service 暴露端口 `19080`。
...
完整指标参考请参阅[指标参考](/zh/1.5/operations/metrics)。
...
| `log.openTelemetry.protocol` | string | `"grpc"` | OTLP 协议（`grpc` 或 `http`） |
...
`sampleRatio` 控制生成追踪的请求比例，默认 `1.0`（全量采样）。生产环境可根据可观测性需求和存储容量调低采样比例。
...
* [指标参考](/zh/1.5/operations/metrics) — 所有可用指标的完整参考
* [Grafana 仪表盘](/zh/1.5/operations/grafana) — 预构建的可视化仪表盘
* [告警规则](/zh/1.5/operations/alerting) — 推荐的告警配置
