...
4. **确认和应用**：数据面收到快照后解析并应用到本地的 Pingora 运行时，然后向控制面发回 ACK。此时数据面已用最新配置开始路由流量。

5. **配置更新**：当 Kubernetes 资源变更时（Gateway、HTTPRoute、Service、EndpointSlice 任一发生变化），控制面重建快照——内部可能是全量或部分重建——然后推送带有新版本号的完整快照。数据面应用后再次发 ACK。线上传输的始终是全量快照，而非增量差分。

6. **重连**：如果 gRPC 流因网络问题或控制面重启而断开，数据面以指数退避重新连接。重连后控制面根据数据面上报的版本号，推送当前的完整快照（若版本已一致则跳过推送）。
...
Nantian Gateway 采用带版本号的全量快照交付配置：

**初始同步。** 数据面首次连接时，控制面构建并推送一份完整的 SotW 快照，覆盖所有资源类型：listener、cluster、route、endpoint。快照带有一个 nonce（唯一标识符），数据面必须在 ACK 中原样回传。

**快照推送。** 初始同步之后，控制面为每个已连接的数据面跟踪一个版本号。当 Kubernetes 资源变化时，快照构建器重建快照（内部可能是全量或部分重建），然后推送带有新版本号的完整快照。版本号让控制面可以跳过向已持有当前版本的数据面推送；线上传输的始终是全量快照，而非增量差分。

**触发条件。** 以下资源的变更会触发快照重建：Gateway（新增或修改入口点）、HTTPRoute（路由规则变更）、Service（后端端口或选择器变更）、EndpointSlice（后端 IP 新增或移除）。

**ACK/NACK 协议。** 每份快照都带有 nonce。数据面必须回复 ACK（回传 nonce）或 NACK（回传 nonce 加错误描述）。ACK 表示快照已解析并应用。NACK 表示某部分被拒绝，上一份版本仍在使用。
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `xds.serverAddress` | string | `http://127.0.0.1:18080` | 控制面 gRPC 地址 |
| `xdsTls.enabled` | bool | `false` | 是否给 xDS 传输加 TLS |
| `xdsTls.caPath` | string | 空 | 验证控制面证书的 CA 证书路径 |
| `xdsTls.certPath` | string | 空 | 客户端证书路径（mTLS） |
| `xdsTls.keyPath` | string | 空 | 客户端私钥路径（mTLS） |

数据面启动后立即尝试连接控制面。如果连接失败，数据面以指数退避重试，直到连接成功。连接建立后，数据面发送节点标识和当前配置版本，控制面据此决定是推送当前的完整快照，还是在版本已一致时跳过推送。
...
数据面启动时如果日志里有 `connection refused`，先检查控制面的 gRPC 服务端是否在预期的端口监听。确认数据面配置中的 `xds.serverAddress` 与控制面的 gRPC 监听地址一致（该地址在控制面配置文件中设置；manager 二进制只接受 `--config`，没有 `--grpc-port` 参数）。同时确认数据面到控制面之间的防火墙或 NetworkPolicy 没有阻断该端口。

### TLS 握手失败

TLS 握手失败通常是证书不匹配。检查 `xdsTls.caPath` 是否指向了签发控制面证书的 CA。对于 mTLS 场景，确认 `xdsTls.certPath` 和 `xdsTls.keyPath` 都已设置，且控制面信任客户端的 CA。可以用 `openssl s_client -connect <cp-host>:<grpc-port>` 手动测试 TLS 握手。
