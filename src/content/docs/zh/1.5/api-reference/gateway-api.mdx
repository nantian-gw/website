...
slug: zh/1.5/api-reference/gateway-api
---

Nantian Gateway 实现了 [Kubernetes Gateway API v1.5.1](https://gateway-api.sigs.k8s.io/) 规范，声明支持 54 个特性。本页列出所有支持的资源类型、核心字段和特性覆盖情况。
...
Nantian Gateway 声明支持 Gateway API v1.5.1 规范中的 54 个特性。以下按类别列出关键特性：
...
| 特性 | 状态 |
|------|------|
| HTTPRouteQueryParamMatching | 支持 |
| HTTPRouteMethodMatching | 支持 |
...
* 查看 [自定义 CRD](/zh/1.5/api-reference/crds) 了解 Nantian Gateway 的扩展资源
* 查看 [快速开始](/zh/1.5/getting-started/quick-start) 部署第一个网关
* 查看 [第一个路由](/zh/1.5/getting-started/first-route) 创建你的第一条路由规则
