...
```bash
# 通过控制面代理查询指定数据面节点
curl http://<controlplane>:18081/v1/dataplanes/<node_id>/summary
```
...
3. 如果是自定义清单，确认每个副本的运行时 `node_id` 是否唯一。

**常见原因**：

- 多个数据面副本共用了同一个运行时 `node_id`
- 老清单把 `dataplane.config.node_id` 固定成同一个值，例如 `dp-kubernetes`
- 仍在使用已经废弃的 `PGW_NODE_ID`，而不是当前 chart 使用的 `NANTIAN_GW_NODE_ID`

**解决方法**：确保每个数据面副本都有唯一的运行时身份。当前 Helm chart 会把 Pod `metadata.name` 注入到 `NANTIAN_GW_NODE_ID`，通常不需要额外修改；不要再用共享固定值覆盖它。若你维护的是自定义清单，请移除 `PGW_NODE_ID`，把 `dataplane.config.node_id` 仅作为兜底值保留，然后重启数据面 Deployment。

### 会话保持使用自动生成密钥的告警

**症状**：数据面日志出现 `session persistence is using an ephemeral, auto-generated secret` 或 `configure shared_secret or sharedSecretFile`。

**解决方法**：多副本部署时请显式配置 `dataplane.session_persistence.existing_secret` 或 `dataplane.session_persistence.shared_secret`。当前 Helm chart 会挂载对应 Secret，并自动把运行时 `session_persistence.secret_key_file` 配好，这样会话保持状态才能跨重启和跨副本共享。
