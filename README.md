...
Mermaid diagrams render client-side in the browser (via the `pre-mermaid` rehype
strategy), so no headless browser is needed at build time. CI runs `npm run build:ci`
